...
def understand_query(
    *,
    ...
    if re.search(r"^\s*(when\b|at\s+what\s+(?:time|point)\b|give\s+me\s+the\s+timestamp\b)", lowered):
        query_types.add("exact_timestamp")
    if any(_contains_cue(lowered, cue) for cue in VISUAL_CUES):
        query_types.add("visual_memory")
    if any(_contains_cue(lowered, cue) for cue in OCR_CUES):
        query_types.add("ocr_or_slide_text")
    if any(_contains_cue(lowered, cue) for cue in AUDIO_CUES):
        query_types.add("audio_memory")
    if any(_contains_cue(lowered, cue) for cue in ACTION_CUES):
        query_types.add("action_memory")
    if any(_contains_cue(lowered, cue) for cue in TEMPORAL_CUES):
    ...
    if any(_contains_cue(lowered, cue) for cue in SYSTEM_CUES):
        query_types.add("system_or_help")
    if any(_contains_cue(lowered, cue) for cue in GENERAL_CUES):
    ...
    if re.search(
        r"\b(who (?:is|was) (?:the )?(?:speaker|lecturer|presenter)|"
        r"which speaker|speaker (?:identity|name)|who said|whose voice|"
        r"what did (?:the )?(?:speaker|lecturer|presenter) say)\b",
        lowered,
    ):
    ...
    visual_hints = sorted({cue for cue in VISUAL_CUES if _contains_cue(lowered, cue)})
    actions = sorted({cue for cue in ACTION_CUES if _contains_cue(lowered, cue)})
    temporal_relations = sorted({cue for cue in TEMPORAL_CUES if _contains_cue(lowered, cue)})
...
def _contains_cue(lowered_text: str, cue: str) -> bool:
    if " " in cue:
        return cue in lowered_text
    return bool(re.search(rf"\b{re.escape(cue)}\b", lowered_text))


def _extract_entities(text: str) -> list[str]:
    question_words = {"I", "What", "Where", "Who", "When", "Why", "How", "Which", "Can", "Does", "Did", "Is", "Are", "Write"}
    entities = set(re.findall(r"\b[A-Z][A-Za-z0-9]*(?:\s+[A-Z][A-Za-z0-9]*)*\b", text))
    acronyms = set(re.findall(r"\b[A-Z]{2,}\b", text))
    return sorted((entities | acronyms) - question_words)
