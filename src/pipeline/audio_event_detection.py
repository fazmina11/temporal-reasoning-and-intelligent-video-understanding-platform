...
AUDIO_EVENT_SCHEMA_VERSION = "audio-events-v1"
...
def build_audio_event_artifacts(
    *, repo_root: Path, video_id: str, window_ms: int = 2000, hop_ms: int = 1000
    ...
    payload = {
        ...
        "config": {"window_ms": window_ms, "hop_ms": hop_ms, "silence_threshold_dbfs": round(silence_threshold, 3)},
...
def _merge_windows(windows: list[dict[str, Any]], maps: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in windows:
        if groups and groups[-1][-1]["label"] == row["label"] and row["start_ms"] <= groups[-1][-1]["end_ms"]:
            groups[-1].append(row)
        else:
            groups.append([row])
    events = []
