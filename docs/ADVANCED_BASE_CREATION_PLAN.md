...
- forced maximum-duration boundaries
- transcript sentence and pause alignment
- scene-cut and visual-difference hints
- canonical non-overlapping atomic spans

Recommended base:

```text
minimum atom: 3 seconds
target atom: 8 seconds
maximum atom: 15 seconds
hard maximum atom: 20 seconds
```
...
- extract scored multimodal boundary candidates
- create canonical non-overlapping atomic spans
- add valid previous/next atom IDs
- write atom JSON
- guarantee no timeline gaps or overlaps
- add fail-closed atom validation report
...
- first atom starts at exactly 0 ms
- last atom reaches the normalized video duration
- every atom has integer start/end milliseconds
- every atom has valid previous/next links
- validation reports zero gaps and zero overlaps
...
- atoms link to previous and next atoms
